...
interface ISpendingRateOracle {
  ...
  function getFaucetTargetBalance(address _faucet) external view returns(uint256);
  function getFaucetTokenAddress(address _faucet) external view returns(address);
}
