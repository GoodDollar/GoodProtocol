...
contract Identity is IdentityAdminRole, SchemeGuard, Pausable {
    ...
    constructor() public SchemeGuard(Avatar(address(0x0))) {}
    ...
    function authenticate(address account)
        ...
        whenNotPaused
    {
        dateAuthenticated[account] = block.timestamp;
    }
    ...
    function isWhitelisted(address account) public view returns (bool) {
        uint256 daysSinceAuthentication = (block.timestamp.sub(dateAuthenticated[account])) / 1 days;
        return
    ...
    function _addWhitelisted(address account) internal {
        ...
        whitelistedCount += 1;
        dateAdded[account] = block.timestamp;
        dateAuthenticated[account] = block.timestamp;
