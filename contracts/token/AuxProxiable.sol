...
abstract contract AuxProxiable is Initializable {
	...
	function implementsFn(bytes4 selector) external pure virtual returns (bool);
	...
	function _updateCodeAddress(address newAddress) internal {
		...
		require(
			AuxUtils.implementation() != address(0),
			"AuxProxiable: not upgradable"
		);
		require(
			proxiableAuxUUID() == AuxProxiable(newAddress).proxiableAuxUUID(),
			"AuxProxiable: not compatible logic"
		);
		require(address(this) != newAddress, "AuxProxiable: proxy loop");
