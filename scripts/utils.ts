...
export const bulkIsWhitelisted = async (
  accounts: Array<String>,
  rpcProvider = new ethers.providers.JsonRpcProvider("https://forno.celo.org"),
  identityAddress = "0xC361A6E67822a0EDc17D899227dd9FC50BD62F42"
) => {
  ...
  setMulticallAddress(50, "0xefec26f51f58016c22c57e3ceb8712efc6ba3354");
  const ethcallProvider = new Provider(rpcProvider, (await rpcProvider.getNetwork()).chainId);
  const identityContract = new Contract(identityAddress, Identity.abi);
...
export const bulkLastAuth = async (accounts: Array<String>, olderThan = undefined) => {
  setMulticallAddress(122, "0x3CE6158b7278Bf6792e014FA7B4f3c6c46fe9410");
  setMulticallAddress(42220, "0x75F59534dd892c1f8a7B172D639FA854D529ada3");
  ...
  const result = await ethcallProvider.all(calls);
  // console.log({ result });
  const whitelisted = accounts.filter((v, i) => parseInt(result[i]) && (!olderThan || parseInt(result[i]) < olderThan));
  return whitelisted;
};
