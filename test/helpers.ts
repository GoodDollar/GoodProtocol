...
export const createDAO = async () => {
  ...
  const goodBuySellFactory = await ethers.getContractFactory("GoodBuySell");
  const goodBuySell = await upgrades.deployProxy(
    goodBuySellFactory,
    [nameService.address],
    { kind: "uups" }
  );
  await goodBuySell.setAddresses();
  await setDAOAddress("GOOD_BUY_SELL", goodBuySell.address);
