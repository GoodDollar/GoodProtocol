...
describe("UBIScheme Upgrade Process", () => {
  ...
  let reputation;
  let founder, claimer1, claimer2, signers, ubiScheme;
  before(async () => {
  ...
  it("should deploy the ubi", async () => {
    ...
    expect(gdBalance.toString()).to.be.equal("1000000");
  });
  ...
  it("should have set new firstclaim await correctly", async () => {
    expect(await firstClaimPool.claimAmount().then(_ => _.toNumber())).to.equal(
      1000
    );
  });
